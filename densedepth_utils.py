...
def load_execute_names(model, zipfile_orig, name_list):
    ...
    converted_img = display_images_new(outputs.copy(),name_list)
    print("Received output shape:{} converted:{}".format(outputs.shape, converted_img.shape))

def loop_load_execute(model, zipfile_orig, input_zip, batch_start, batch_end):
    print('Loading input data....')
    name_list = [name for name in input_zip[batch_start:batch_end]]
    image_list = [Image.open(BytesIO((zipfile_orig.read(name)))) for name in input_zip[batch_start:batch_end] ]   
        # img = 
...
def display_images_new(outputs, input_names_list ):
    is_colormap=True 
    is_rescale=True
    ...
    for i in range(outputs.shape[0]):
        ...
        file_name = os.path.join('/content/test/',"depth_"+os.path.basename(input_names_list[i]))
...
# def save_images_output(outputs, inputs=None, gt=None, is_colormap=True, is_rescale=False):
#     for i in range(outputs.shape[0]):
